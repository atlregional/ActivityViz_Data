...
hhsize_dt = od_dt[SUBMITTAL!="Dummy",.(COUNT = round(sum(Updated_LINKED_WGHT_FCTR),2),
...
# Check the field name
# od_dd_dt[grepl("household", DESCRIPTION, ignore.case = TRUE)]
income_dt = od_dt[SUBMITTAL!="Dummy",.(COUNT = round(sum(Updated_LINKED_WGHT_FCTR),2),
...
gender_dt = od_dt[SUBMITTAL!="Dummy",.(COUNT = round(sum(Updated_LINKED_WGHT_FCTR),2),
...
num_veh_dt = od_dt[SUBMITTAL!="Dummy",.(COUNT = round(sum(Updated_LINKED_WGHT_FCTR),2),
...
have_dl_dt = od_dt[SUBMITTAL!="Dummy",.(COUNT = round(sum(Updated_LINKED_WGHT_FCTR),2),
...
# have_dl_dt = od_dt[!is.na(`HAVE_DL.Code.`) & `HAVE_DL.Code.`!= "",.(COUNT = round(sum(Updated_LINKED_WGHT_FCTR),2)), by = .(HAVE_DL, `HAVE_DL.Code.`)]
...
fare_method_dt = od_dt[,.SD,.SDcols=c("ID", "Updated_LINKED_WGHT_FCTR", 
                                      grep("FARE_METHOD", names(od_dt), value = TRUE))]
fare_method_long_dt = melt.data.table(fare_method_dt, id.vars = c("ID", "Updated_LINKED_WGHT_FCTR"))
fare_method_long_dt = fare_method_long_dt[!value %in% c("")]
fare_method_long_dt[,FARE:=fare_labels[.(variable),.(LABEL)]]
fare_method_dt = fare_method_long_dt[,.(COUNT = round(sum(Updated_LINKED_WGHT_FCTR),2),
...
use_service_dt = od_dt[,.SD,.SDcols=c("ID", "Updated_LINKED_WGHT_FCTR", grep("USE_SERVICES_SE_MI", names(od_dt), value = TRUE))]
use_service_long_dt = melt.data.table(use_service_dt, id.vars = c("ID", "Updated_LINKED_WGHT_FCTR"))
use_service_long_dt = use_service_long_dt[!value %in% c("")]
use_service_long_dt[,SERVICE:=service_labels[.(variable),.(LABEL)]]
use_service_dt = use_service_long_dt[,.(COUNT = round(sum(Updated_LINKED_WGHT_FCTR),2),
